...
def save_points_to_csv_c(points, filename, columns):
    """
    ...
    try:
        ...
        directory = 'classic/' + filename.split('.')[0] + '(simplified).csv'
...
def save_points_to_csv_p(points, filename, columns):
    """
    Turns the new parallelized rdp points into a csv file and saves it in the /simplified directory
    """
    try:
        ...
        # save the parallelized points into a new csv file
        directory = 'parallel/' + filename.split('.')[0] + '(simplified).csv'
        df = pd.DataFrame({columns[0]: col_1, columns[1]: col_2})
        df.to_csv(directory, index=False)
        return 1
    except:
...
filename = input("Enter CSV file path (e.g. Datasets/Boeing.csv): ")
...
with open(filename, 'r') as file:
    ...
    # file sizes
    save_file_c = save_points_to_csv_c(
        points=classic_points, filename=filename[12:], columns=cols)
    directory_c = 'classic/' + \
        filename[12:].split('.')[0] + '(simplified).csv'

    # file sizes
    save_file_p = save_points_to_csv_p(
        points=parallelized_points, filename=filename[12:], columns=cols)
    directory_p = 'parallel/' + \
        filename[12:].split('.')[0] + '(simplified).csv'
    ...
    if (save_file_c):
        classic_file_size = get_file_size(directory_c)

    if (save_file_p):
        parallel_file_size = get_file_size(directory_p)
    ...
    t_statistic_c, p_value_c = ttest_ind([point[1] for point in points], [
                                     point[1] for point in classic_points])

    # calculate the t statistic
    t_statistic_p, p_value_p = ttest_ind([point[1] for point in points], [
    ...
    print('File size of classic line : ' +
          str(classic_file_size) + ' KB')
    print('File size of parallel line : ' +
    ...
    print('\nT statistic classic : ' +
          str(t_statistic_c))
    print('T statistic parallel : ' +
          str(t_statistic_p))
    print('\nP value classic : ' +
          str(p_value_c))
    print('P value parallel : ' +
          str(p_value_p))
    ...
    if p_value_c >= tol:
        print('\nClassic Result : There is no significant difference between the two lines')
    else:
        print(
            '\nClassic Result : There is a significant difference between the two lines')
    if p_value_p >= tol:
        print('Parallel Result : There is no significant difference between the two lines\n')
    else:
        print(
            'Parallel Result : There is a significant difference between the two lines\n')